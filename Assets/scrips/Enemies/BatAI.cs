...
public class BatAI : MonoBehaviour
{
    ...
    // {
    //     var self = GetComponent<Rigidbody2D>();
    //     Vector2 targetDirection = (playerLocked.transform.position - transform.position).normalized;
    //     float angle = 45f; // 设置偏转角度，可以根据需要调整
    //     float cosAngle = Mathf.Cos(angle * Mathf.Deg2Rad);
    //     float sinAngle = Mathf.Sin(angle * Mathf.Deg2Rad);
    //     Vector2 rotatedDirection = new Vector2(targetDirection.x * cosAngle - targetDirection.y * sinAngle, targetDirection.x * sinAngle + targetDirection.y * cosAngle);
    //     self.velocity = rotatedDirection * 3;
    //     yield return new WaitForSeconds(0.5f);
    //     self.velocity = (spawnPoint - (Vector2)transform.position) * 6;
    //     yield return new WaitForSeconds(0.5f);
    //     StartCoroutine(FlyAround(spawnPoint));
